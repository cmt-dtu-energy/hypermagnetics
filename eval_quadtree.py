...
for test_sources in range(0, int(n_eval), int(step_sources)):
    ...
    for i in range(n_ensemble):
        ...
        if i == 0:
            msp, field_fmm = potential2D_sources(test["sources"][i : i + 1], "prism")
        # Run model for potential
        start_time_pot = time.time()
        msp, field_fmm = potential2D_sources(test["sources"][i : i + 1], "prism")
        # msp, field_fmm = potential2D(test["sources"][i : i + 1], test["r"], "prism")
        pot_out[i] = msp
...
color = "tab:red"
ax1.set_xlabel("Number of sources (*1e3)")
